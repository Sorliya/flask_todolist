...
def index():
    ...
    posts = Post.query.order_by(Post.create_at.desc()).paginate(page, 2, False)
...
def user_page(username):
    ...
    if user:
        page = request.args.get('page', 1, type=int)
        posts = Post.query.filter_by(user_id=user.id).order_by(Post.create_at.desc()).paginate(page, 2, False)
...
def edit_post(id):
    ...
    if form.validate_on_submit():
        ...
        post.currency=request.form['currency']
        posts=Post(name=name,description=description,currency=currency)
        db.session.commit()
